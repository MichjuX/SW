module adder_A_B_8_bits_on_board(
	...
	wire [7:0] A, B, S;
	wire cout;
	reg_N_bits_with_areset reg_A(KEY[0], ~KEY[1], SW, A);
	reg_N_bits_with_areset reg_B(KEY[0], KEY[1], SW, B);
	assign {cout, S} = A + B;
	assign s = S;
...
module reg_N_bits_with_areset
	...
	always @ (clk, reset)
		if (reset) Q = {N{1'b0}};
		// jeżeli clk = 1, to zatrzask przepuszcza D na Q
		// jeżeli clk = 0, to Q jest zatrzaśnięte, czyli zachowuje wartość, która była na Q w chwili zatrzaśnięcia
		else if (clk == 1'b1)
			Q = D;
