module real_time_clock_on_board(
	...
	wire c0;
	delay_10ms delay(CLOCK_50, c0);
	wire [7:0] m, s, cs /* synthesis keep */;
	real_time_clock rtc(c0, ~KEY[0], ~KEY[3], ~KEY[2], ~KEY[1],
		SW, SW, SW,
		m, s, cs);
...
module real_time_clock(
	input c0, toggle_enable, m_aload, s_aload, cs_aload,
	// minuty, sekundy, centysekundy; na każdą jednostkę czasu 2 cyfry BCD po 4 bity
	input [7:0] m_data, s_data, cs_data,
	output [7:0] m, s, cs);
	...
	counter_modulo_k #(10) count_cs0(c0, 1'b1, enable, cs_aload, cs_data[3:0], h0, c1);
	counter_modulo_k #(10) count_cs1(~c1, 1'b1, 1'b1, cs_aload, cs_data[7:4], h1, c2);
	counter_modulo_k #(10) count_s0(~c2, 1'b1, 1'b1, s_aload, m_data[3:0], h2, c3);
	counter_modulo_k #(6) count_s1(~c3, 1'b1, 1'b1, s_aload, m_data[7:4], h3, c4);
	counter_modulo_k #(10) count_m0(~c4, 1'b1, 1'b1, m_aload, m_data[3:0], h4, c5);
	counter_modulo_k #(6) count_m1(~c5, 1'b1, 1'b1, m_aload, m_data[7:4], h5);
