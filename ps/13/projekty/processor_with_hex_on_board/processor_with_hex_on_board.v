module processor_with_hex_on_board(
	input [9:9] SW,
	input [0:0] KEY,
	input CLOCK_50,
	output [6:0] HEX0, HEX1, HEX2, HEX3);

	processor_with_hex proc(KEY[0], CLOCK_50, SW[9], HEX0, HEX1, HEX2, HEX3);

endmodule

module processor_with_hex(
	...
	processor_2 proc(DIN, Resetn, Clock, Run, ADDR, DOUT, wr_en);
	// dekoder
	wire [3:0] RH_E;
	decoder2_4 dec(ADDR[8:7], wr_en, RH_E);
	regn #(7) RH0(DOUT[6:0], RH_E[0], Clock, HEX0);
	regn #(7) RH1(DOUT[6:0], RH_E[1], Clock, HEX1);
	regn #(7) RH2(DOUT[6:0], RH_E[2], Clock, HEX2);
	regn #(7) RH3(DOUT[6:0], RH_E[3], Clock, HEX3);
...
module decoder2_4(
	...
	assign out[0] = (~s[1]) & (~s[0]) & enable;
	assign out[1] = (~s[1]) & s[0] & enable;
	assign out[2] = s[1] & (~s[0]) & enable;
	assign out[3] = s[1] & s[0] & enable;
